...
/**
 * CommonDecoder继承于ReplayingDecoder，与MessageToByteEncoder相反，它用于将收到的字节序列还原实际对象。
 * 主要就是一些字段的校验，比较重要的就是取出序列化器的编号，以获得正确的反序列化方式，并且读入length字段来确定
 * 数据包的长度(防止粘包)，最后读入正确大小的字节数组，反序列化成对应的对象。
 */
